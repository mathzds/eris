...
async function loadCommands() {
	...
	return () => {
		...
		try {
			...
			console.debug(
				`[${commands.length}].purple commands has been registered!`,
			);
		} catch (error) {
...
export default async () => {
	...
	client.on("interactionCreate", async (interaction) => {
		await database.getRepository(User).findOneBy({
			...
			if (!user) {
				...
				if (user.discordUsername !== interaction.user.username) {
					database.getRepository(User).update(user.discordID, {
						discordUsername: interaction.user.username,
					});
				}
			}
	...
	try {
		...
		console.error("An error has occurred while trying to connect to discord!", error);
	}
};
